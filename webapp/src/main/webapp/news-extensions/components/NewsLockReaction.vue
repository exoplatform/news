<template>
  <div v-if="displayLockMessage" class="lockReaction d-flex flex-row mt-lg-1 ms-4 disabled--text">
    <v-icon
      small
      class="baseline-vertical-align mx-auto disabled--text">
      mdi-lock
    </v-icon>
    <span v-if="!isMobile" class="ms-2 mx-auto">{{ $t('news.details.cannot.react') }}</span>
  </div>
</template>
<script>
export default {
